	...
	<div class = "content">
		<div class = "content_wrapper" style = "margin-top: 5px;">
			<div class = "inner_conten" style = "width: 1000px; margin: 0 auto;">
				...
				*/
					$_SESSION['tutor'];
					$query = mysql_query("SELECT cafedras.cafedraID, cafedras.FacultyID, tutors.TutorID
					FROM cafedras
					INNER JOIN faculties ON faculties.FacultyID = cafedras.FacultyID
					INNER JOIN tutors ON tutors.CafedraID = cafedras.cafedraID
					WHERE Login = '$_SESSION[tutor]'") or die(mysql_error());
					$tutor = mysql_fetch_array($query);
					
					function load_korsetkish(){
						$output = '';
						$sql = "SELECT * FROM korsetkishter WHERE bolimderID IN(1,6)";
				...
				<div class = "select_box">
					<form id="form1" method = "post" action = "load_engbek.php" style = "margin-top: 10px;" enctype = "multipart/form-data">
						...
						<input type = "number" id = "univ_avtor_san" name = "univ_avtor_san" onkeypress='validate(event)' value = "1" min="1"/>
						<br /><br />
						Еңбек санының түрлері
						<select id = "select_sany", disabled = true;>
							<option value = "0">---</option>
							<option value = "1">Баспа табақ саны</option>
							<option value = "2">Жалпы соммасы (Млн. теңге)</option>
							<option value = "3">Сағат саны</option>
							<option value = "4">Шаршы см.</option>
							<option value = "5">Саны</option>
						</select><br /><br />
						<label for="sany" id = "label_sany">---</label><br /><input type = "number" id = "sany" name = "sany" value = "1" step="0.01" min="0.01"><br /><br />		
						...
						<span>Растаушы файлды таңдау (PDF, JPG форматындағы файлдар (10 мб төмен))</span><br/><br/>
						...
						<input type = "submit" name = "upload000" value = "Жүктеу"/>
					</form>
				</div>
				<div class = "works">
					<style>
