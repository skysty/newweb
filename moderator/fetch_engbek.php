...
/*	$sql = "SELECT * FROM state WHERE countryID = '".$_POST["countryID"]."' AND stateID = '".$_POST["stateID"]."'";
    ...
    if (!empty($_POST["FacultyID"]))
        $where .= ' AND engbekter.kod_fakul = "' . $_POST["FacultyID"].'"';
	...
	<th>Автор саны</th>
	<th>Файл аты</th>
	<th>Балл</th><th>Қайтару себебі</th><th>Статус</th><th>Толығырақ</th></tr>";
    ...
    while($row = mysqli_fetch_array($result)){
		...
		<td><a target='_blank' href = \"engbek.php?ID=" . $row['engbekID'] . "\">Тексеру</a></td>
		</tr>";
		$i++;		
    } 
