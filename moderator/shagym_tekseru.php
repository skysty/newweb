...
<head>
	...
	?>
	<title>Шағымдарды тексеру</title>
	...
	<div class = "content">
		<div class = "content_wrapper" style = "margin-top: 5px;">
			<div class = "shagymTable">
				...
				<table>
					<tr>
						<th>№</th><th>Еңбек иесі</th><th>Еңбек</th><th>Шағым</th><th>Шағым күні</th><th>Тексеру нәтижесі</th><th>Жауап жазу</th>
					</tr>
					<?php
						
						$query = mysqli_query($connection,"SELECT shagymdar.shagymID, shagymdar.shagym, shagymdar.shagym_kuni, shagymdar.jauap, shagymdar.jauap_kuni, engbekter.file_ati, tutors.firstname, tutors.lastname
						FROM shagymdar
						...
						while($shagym = mysqli_fetch_array($query)){
							echo "<tr><td>" . $i ."</td><td>".$shagym['lastname']." ".$shagym['firstname']."</td><td>".$shagym['file_ati']."</td><td>". $shagym['shagym'] .
							"</td><td>". $shagym['shagym_kuni'] .
							"</td><td>". $shagym['jauap'] ."</td><td><a href = \"update_shagym.php?ID=" . $shagym['shagymID'] . "\">Жауап жазу</a></td></tr>";
							$i++;
						}
