		...
		<div class = "content_wrapper" style = "margin-top: 5px;">					
			...
			*/
			...
			<div class = "cafedra_table">
						<?php
							$doc_sql = mysqli_query($connection,"SELECT tutors.RATE, tutors.job_titleID, tutors.lastname, tutors.TutorID, tutors.firstname, SUM(engbekter.ball) AS sum_val FROM engbekter RIGHT JOIN tutors ON tutors.TutorID = engbekter.kod_kizm
						...
						<table>
							...
							<?php
								$i = 1;
								while($tutor = mysqli_fetch_array($doc_sql)){
									$i++;
					...
					<div class = "cafedra_table">
						<?php
							$kan_sql = mysqli_query($connection,"SELECT tutors.RATE, tutors.job_titleID, tutors.lastname, tutors.TutorID, tutors.firstname, SUM(engbekter.ball) AS sum_val FROM engbekter RIGHT JOIN tutors ON tutors.TutorID = engbekter.kod_kizm
						...
						<table>
							...
							<?php
								$i = 1;
								while($tutor = mysqli_fetch_array($kan_sql)){
									$i++;
									$sum_val = sprintf("%.2f", $tutor['sum_val']);
									echo "<tr><td>".$i."</td><td>".$tutor['lastname']." ".$tutor['firstname']."</td><td>" . $sum_val . "</td><td><a href = \"tolyk.php?ID=" . $tutor['TutorID'] . "\">Толық >></a></td></tr>";			
								}
			...
			<div class = 'tutors_table'><h3 align = 'center' style = "color: #0094de">Оқытушылар</h3>
			<?php
				$sql1 = mysqli_query($connection,"SELECT tutors.RATE, tutors.job_titleID, tutors.lastname, tutors.TutorID, tutors.firstname, SUM(engbekter.ball) AS sum_val FROM engbekter RIGHT JOIN tutors ON tutors.TutorID = engbekter.kod_kizm
