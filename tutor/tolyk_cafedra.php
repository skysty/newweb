	...
	<div class = "content">
		<div class = "content_wrapper" style = "width: 100%; margin: 0 auto; margin-top: 10px;">
			...
			<?php
				...
				$sql3 = mysqli_query($connection,"SELECT engbekter.engbekID, SUM(engbekter.ball) AS sum_gylym1,SUM(engbekter.ball)*0.55 AS sum_gylym, engbekter.file_ati, engbekter.sani, cafedras.cafedraNameKZ, korsetkishter.korsetkish_ati, cafedras.cafedraID, korsetkishter.typeID
				FROM engbekter
				...
				$sql4 = mysqli_query($connection,"SELECT engbekter.engbekID,  SUM(engbekter.ball) AS sum_oku, SUM(engbekter.ball)*0.35 AS sum_oku, engbekter.file_ati, engbekter.sani, cafedras.cafedraNameKZ, korsetkishter.korsetkish_ati, cafedras.cafedraID, korsetkishter.typeID
				FROM engbekter
				INNER JOIN cafedras ON cafedras.cafedraID = engbekter.kod_kafedra
				INNER JOIN korsetkishter ON korsetkishter.kod_korsetkish = engbekter.kod_korset
			...
			<div class = "engbek">
				<form action = "confirmation.php" method = "post">
					<table style = "font-size: 19px;">
						...
						<tr>
							<td>
                                                          <strong>Академиялық бағыты:35%</strong>
                                                       </td>
                                                        <td>
                                                        <?php 
                                                         $format='%.2f/%.2f';
						...
						<tr>
							<td>
							  <strong>Әлеуметтік-мәдени бағыты:10%</strong>
							</td>
							<td>
							<?php 
							 $format='%.2f/%.2f';
