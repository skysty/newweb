	...
	<div class = "content">
		<div class = "content_wrapper" style = "margin-top: 5px;">
			<div class = "inner_conten" style = "width: 1000px; margin: 0 auto;">
				<h2 style = "text-align: center;">Факультеттің немесе ҒЗО-ның орындалған жұмыстары</h2>			
				<?php
					$_SESSION['tutor'];
					...
					function load_korsetkish(){
						...
						global $connection;
						$output = '';
						$sql = "SELECT * FROM korsetkishter WHERE bolimderID IN(2) and typeID='$tt'";
				...
				<div class = "select_box">
					<form method = "post" action = "load_engbek_faculty.php" style = "margin-top: 10px;" enctype = "multipart/form-data">
						...
						<label for="sany" id = "label_sany">Саны</label><br /><input type = "number" id = "sany" name = "sany" onkeypress='validate(event)' value = "1" step="0.01" min="0.01"><br /><br />		
				...
				<div class = "works">
					...
					<?php
						...
						$sql = "SELECT engbekter.ball, engbekter.engbekID, tutors.firstname, engbekter.kod_kafedra, tutors.lastname, tutors.patronymic, korsetkishter.korsetkish_ati, 	engbekter.sani, engbekter.univ_avtor_san, engbekter.file_ati, engbekter.kayt_sebeb, engbekter.eskertu, status.status_name, faculties.FacultyID, status.statusID,  faculties.facultyNameKZ  
						FROM engbekter 
						...
						WHERE Login = '$tutor_id' and engbekter.kod_kafedra IS NULL   ORDER BY engbekter.engbekID DESC";
						
						$result = mysqli_query($connection, $sql) or die(mysqli_error($connection));
						
						echo "<table><tr><th>№</th><th>Факультет/ҒЗИ</th><th>Аты жөні</th><th>Көрсеткіш</th><th>Саны</th><th>Автор саны</th><th>Файл аты</th><th>Балл</th><th>Қайтару себебі</th><th>Статус</th></tr>";
						
						$i = 1;
						
						while($row = mysqli_fetch_array($result)){
							echo "<tr><td>".$i."</td><td>".$row["facultyNameKZ"]."</td><td>".$row["lastname"]." ".$row["firstname"]."</td><td>".$row["korsetkish_ati"]."</td><td>".$row["sani"]."</td><td>".$row["univ_avtor_san"]."</td><td><a target='_blank' href = " .$row['file_ati'] .">".$row["file_ati"]."</a></td><td>".$row["ball"]."</td><td>".$row["kayt_sebeb"]."</td><td>".$row["status_name"]."</td></tr>";
							$i++;
						}
