	...
	<div class = "content">
		<div class = "content_wrapper" style = "width: 100%; margin: 0 auto; margin-top: 10px;">
			...
			<?php
					...
					$sql3 = mysqli_query($connection,"SELECT engbekter.engbekID, SUM(engbekter.ball*0.50) AS sum_gylym, engbekter.file_ati, engbekter.sani, cafedras.cafedraNameKZ, korsetkishter.korsetkish_ati, cafedras.cafedraID, korsetkishter.typeID
					FROM engbekter
					...
					$sql4 = mysqli_query($connection,"SELECT engbekter.engbekID, SUM(engbekter.ball*0.35) AS sum_oku, engbekter.file_ati, engbekter.sani, cafedras.cafedraNameKZ, korsetkishter.korsetkish_ati, cafedras.cafedraID, korsetkishter.typeID
					FROM engbekter
					INNER JOIN cafedras ON cafedras.cafedraID = engbekter.kod_kafedra
					INNER JOIN korsetkishter ON korsetkishter.kod_korsetkish = engbekter.kod_korset
					...
					$sql5 = mysqli_query($connection,"SELECT engbekter.engbekID, SUM(engbekter.ball*0.15) AS sum_tarbie, engbekter.file_ati, engbekter.sani, cafedras.cafedraNameKZ, korsetkishter.korsetkish_ati, cafedras.cafedraID, korsetkishter.typeID
					FROM engbekter
					INNER JOIN cafedras ON cafedras.cafedraID = engbekter.kod_kafedra
					INNER JOIN korsetkishter ON korsetkishter.kod_korsetkish = engbekter.kod_korset
					...
					$univ = array_sum($a);
					$sql2 = mysqli_query($connection,"SELECT engbekter.engbekID, SUM(engbekter.ball) AS sum_val, $univ/cafedras.shtat_sany AS avg_cafedra, cafedras.cafedraNameKZ, cafedras.cafedraID, tutors.lastname, tutors.TutorID, tutors.firstname, tutors.patronymic
					FROM cafedras
			...
			<div class = "engbek">
				<form action = "confirmation.php" method = "post">
					<table style = "font-size: 19px;">
						...
						<tr>
							<td><strong>Ғылым бағыты:</strong></td><td><?php echo sprintf("%.2f",$cafedra2['sum_gylym']); ?></td>
						</tr>
						<tr>
							<td><strong>Академиялық бағыты:</strong></td><td><?php echo sprintf("%.2f",$cafedra3['sum_oku']); ?></td>
						</tr>
						<tr>
							<td><strong>Әлеуметтік-мәдени бағыты:</strong></td><td><?php echo sprintf("%.2f",$cafedra4['sum_tarbie']); ?></td>
						</tr>
						<tr>
							<td><hr/></td><td><hr /></td>
						</tr>
					...
					<?php
						...
						while($row = mysqli_fetch_array($result))
						{
							...
							<tr>
								...
								<td>".$row["korsetkish_ati"]."</td><td>".$row["sani"]."</td>
								<td>".$row["univ_avtor_san"]."</td>
								<td><a target='_blank' href = " .$row['file_ati'] .">".$row["file_ati"]."</a></td><td>".$row["ball"]."</td><td>".$row["kayt_sebeb"]."</td><td>".$row["status_name"]."</td>
							</tr>";
							$i++;
						}
